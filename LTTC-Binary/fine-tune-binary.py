...
def main():
    ...
    parser.add_argument(
        "--num_train_epochs",
        type=int,
        default=5,  # Changed from 1 to 5 as per requirements
        help="Number of complete passes through the training dataset",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
        default=5.0e-5,  # Changed from 4.0e-5 to 5.0e-5 as per requirements
    ...
    parser.add_argument(
        "--eval_metric",
        type=str,
        default="binary",  # Changed from "both" to "binary" for binary classification
    ...
    parser.add_argument(
        "--hub_model_id",
        type=str,
        default="ntnu-smil/Phi-4-mm_Binary_QA_NI_0415",  # Updated default hub_model_id
    ...
    parser.add_argument(
        "--form_id",
        type=str,
        default="1764",  # Added form_id parameter
    ...
    # Update hub_model_id with form_id if it's the default value
    if args.hub_model_id == "ntnu-smil/Phi-4-mm_Binary_QA_NI_0415":
        args.hub_model_id = f"ntnu-smil/Phi-4-mm_Binary_QA_NI_0415_{args.form_id}"
    ...
    if not args.skip_initial_eval:
        ...
        if accelerator.is_main_process:
            ...
            print(f"Binary accuracy: {score.get('binary_accuracy', 'N/A')}")
            print(f"Probability MSE: {score.get('probability_mse', 'N/A')}")
            print(f"Probability accuracy: {score.get('probability_accuracy', 'N/A')}")
            print(f"Combined score: {score.get('combined_score', 'N/A')}")
